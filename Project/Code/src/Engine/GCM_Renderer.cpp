...
void CGCM_Renderer::InitSurfaces() {
  ...
  An offset is the space between from the base address of local memory and a
  certain useable address.
  Offsets are used in gcm commands that deal with shader parameters, texture
...
void CGCM_Renderer::SetCurrentShader(GCM_VertexShader& vert,
...
void CGCM_Renderer::renderMesh(Mesh* const msh, const Matrix4& mvp) {
  ...
  */
  ...
  cell::Gcm::cellGcmSetVertexDataArray(
      defaultVert->VERTEX_POSITION_INDEX,  // index
      0,  // Frequency
  ...
  cell::Gcm::cellGcmSetVertexDataArray(
      defaultVert->VERTEX_COLOUR_INDEX, 0, sizeof(stVertex), 4,
