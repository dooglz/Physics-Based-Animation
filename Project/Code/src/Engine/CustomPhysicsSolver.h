...
class CCustomCubePhysicsObject : public CCustomPhysicsObject {
 public:
  CCustomCubePhysicsObject(float mass, Vector3 position, Quaternion rotation,
                           float lw, float lh, float ld);
};

class CCustomSpherePhysicsObject : public CCustomPhysicsObject {
 public:
  CCustomSpherePhysicsObject(float mass, Vector3 position, Quaternion rotation,
                             float radius);
};
...
class CCustomSover : public CPhysicsSolver {
 ...
 public:
  ...
  void Shutdown();
  CPhysicsObject* CreateCubeObject(float mass, Vector3 position,
                                   Quaternion rotation, float lw, float lh,
                                   float ld);
  CPhysicsObject* CreateSphereObject(float mass, Vector3 position,
