...
CPhysicsObject::CPhysicsObject(const float mass, const Vector3& position,
...
void CPhysicsObject::Integrate(const double dt, const double timestep) {
  ...
  if (_position.getX() * _position.getX() < 28.0f &&
      _position.getZ() * _position.getZ() < 28.0f && _position.getY() < 8.0f) {
    // AddImpulse(Vector3(0.015f*_linVelocity.getX(), 0.05f,
    // 0.0015f*_linVelocity.getZ()));
  }
  ...
  Quaternion Qvel = (Quaternion(0, _angVelocity.getX(), _angVelocity.getY(),
                                _angVelocity.getZ()) *
                     0.5f) *
                    _orientation;
...
Matrix4 CPhysicsObject::GetInvLocalTensor() const { return _localInvInertia; }

Vector3 CPhysicsObject::GetPosition() const { return _position; }

float CPhysicsObject::GetInvMass() const { return _invMass; }
...
Vector3 CPhysicsObject::GetLinearVeloicty() const { return _linVelocity; }

Vector3 CPhysicsObject::GetAngularVeloicty() const { return _angVelocity; }

Quaternion CPhysicsObject::GetOrientation() const { return _orientation; }

Matrix4 CPhysicsObject::GetInvWorldTensor() const { return _worldInvInertia; }
}