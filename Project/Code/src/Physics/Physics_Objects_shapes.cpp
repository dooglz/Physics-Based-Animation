...
CCube_Object::CCube_Object(const float mass, const Vector3& position,
                           const Quaternion& rotation, const float lw,
                           const float lh, const float ld)
    : CPhysicsObject(mass, position, rotation) {
  type = CUBEOID;
...
CSphere_Object::CSphere_Object(const float mass, const Vector3& position,
                               const Quaternion& rotation, const float radius)
    : CPhysicsObject(mass, position, rotation) {
  type = SPHERE;
  _radius = radius;
  SetMass(mass);
...
CPlane_Object::CPlane_Object(const Vector3& position, const Vector3& normal)
    : CPhysicsObject(0.0f, position, Quaternion::identity()) {
  type = PLANE;
  _normal = normal;
  SetMass(0);
  partitioned = false;
...
std::vector<Vector3> CCube_Object::getDebugLines() {
  ...
  aa.push_back(_position +
               Vector3(-_size.getX(), -_size.getY(), -_size.getZ()));
  aa.push_back(_position + Vector3(-_size.getX(), _size.getY(), -_size.getZ()));
  aa.push_back(_position + Vector3(_size.getX(), -_size.getY(), _size.getZ()));
  aa.push_back(_position + Vector3(_size.getX(), _size.getY(), -_size.getZ()));
  aa.push_back(_position + Vector3(-_size.getX(), -_size.getY(), _size.getZ()));
