...
Game::Game() {
  printf(
      "Project for Physics Based Animation\n(SET09119) Edinburgh Napier "
      "University\nSam Serrels\n");
}
...
void Game::Init() {
  ...
  ent1->AddComponent(new Engine::CmPhysics_Cuboid(
      10, ent1->getPosition(), ent1->getRotation(), ent1->getScale().getX(),
      ent1->getScale().getY(), ent1->getScale().getZ()));
  ...
  ent4->AddComponent(new Engine::CmPhysics_Sphere(
      10, ent4->getPosition(), ent1->getRotation(), ent4->getScale().getX()));
  ...
  ent2->AddComponent(
      new Engine::CmPhysics_Plane(Vector3(0, 0, 0), Vector3(0, 1, 0)));
  ...
  for (unsigned int i = 0; i < a; i++) {
    ...
    e->setPosition(Vector3(50.0f * sin(aa * 5.0f),
                           30.0f + 25.0f * sin(aa * 6.0f),
                           -50.0f * cos(aa * 5.0f)));
    ...
    e->AddComponent(new Engine::CmPhysics_Sphere(20, e->getPosition(),
                                                 e->getRotation(), 1.0f));
...
void Game::Update(double delta) {
  ...
  //	Engine::Renderer->DrawLine(Vector3(0.0f), Vector3(3 * sin(a), 3.5f, 3 *
  // cos(a)));
